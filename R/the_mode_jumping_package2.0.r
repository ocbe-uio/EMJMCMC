                             ...
                             learnlocalMCMC=function(model)
                             {
                               ...
                               for(m in 1:M)
                               {
                                 withRestarts(tryCatch({
                                   ...
                                   if(log(stats::runif(n = 1,min = 0,max = 1)) < sum(na.rm = T,sum(na.rm = T,log(sum(na.rm = T,exp(p.select.y))),-log(sum(na.rm = T,exp(p.select.z))), max.p.select.y, - max.p.select.z )))
                                   {
                                     mlikcur<-mlikcand
                                 ...
                                 for(m in 1:M)
                                 {
                                   withRestarts(tryCatch({
                                     ...
                                     for(mod_id in 1:mmax.cpu)
                                     {
                                       ...
                                       {
                                         delta<-objcand - objcur
                                         if(stats::runif(n = 1,min = 0,max = 1) <= sum(na.rm = T, exp(x = -delta/t)))
                                         {
                             ...
                             modejumping_mcmc=function(glob.model)
                             {
                               ...
                               while((eps.emp>=glob.model$eps || j<= glob.model$maxit || j <= glob.model$burnin) && delta.time < glob.model$max.time && g.results[4,1]<= glob.model$trit && g.results[4,2]<= glob.model$trest)
                               {
                                 ...
                                 if(LocImprove>3)
                                 {
                                   ...
                                   if(log(stats::runif(n = 1,min = 0,max = 1)) < sum(na.rm = T,sum(na.rm = T,sum(na.rm = T,log(sum(na.rm = T,exp(p.select.y))),-log(sum(na.rm = T,exp(p.select.z))), max.p.select.y, - max.p.select.z ))))
                                   {
                                     mlikcur<-mlikcand
                                     ratcur<-mlikcand
                                 ...
                                 if(LocImprove<=3)
                                 {
                                   ...
                                   if(LocImprove == 0 || LocImprove == 3)
                                   {
                                     ...
                                     if(LocImprove == 0)
                                     {
                                       ...
                                       if(log(stats::runif(n = 1,min = 0,max = 1))<=sum(na.rm = T, thact))
                                       {
                                         ratcur<-ratcand
                                         mlikcur<-ratcand
