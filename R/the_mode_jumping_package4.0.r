                                 ...
                                 }else if(allow_offsprings  == 4  && j%%mutation_rate == 0 && (j<=last.mutation || Nvars!=Nvars.max))
                                 {
                                   ...
                                   if(Nvars>Nvars.max || j==mutation_rate)
                                   {
                                     ...
                                     if(length(to.del)>0)
                                     {
                                       ...
                                       rm(hashStat)
                                       gc()
                                       #hashStat <- hash::hash(keys=keysarr.new,values=as.list(data.frame((values.new))))
                                       hash::clear(hashStat)
                                       fparam<<-fparam[-to.del]
                                       pool.probs[which(fparam.pool %in% fparam)]<-1
                                       Nvars<<-length(fparam)
                                   ...
                                   else
                                   {
                                     ...
                                     while(idel <= lidmut){
                                       ...
                                       if(action.type==1)
                                       {
                                         ...
                                         {
                                           mother<-ifelse(stats::runif(n = 1,min = 0,max = 1)<=pool.cross,fparam[actvars][sample.int(n = length(actvars),size =1)],fparam.pool[sample.int(n = length(fparam.pool),size =1,prob = pool.probs)])
                                           #ltreem<-stringi::stri_length(mother)
                                           #mother<- stringi::stri_sub(mother,from=1, to = ltreem)
                                           #sjm<-sum(stringi::stri_count_fixed(str = mother, pattern = c("+","*")))
                                           # generate a father
                                           father<-ifelse(stats::runif(n = 1,min = 0,max = 1)<=pool.cross,fparam[actvars][sample.int(n = length(actvars),size =1)],fparam.pool[sample.int(n = length(fparam.pool),size =1,prob = pool.probs)])
                                           #ltreef<-stringi::stri_length(father)
                                           #father<- stringi::stri_sub(father,from=1, to = ltreef)
                                           #sjf<-sum(stringi::stri_count_fixed(str = father, pattern = c("+","*")))
                                         }
                                         ...
                                         {
                                           proposal<-ifelse(stats::runif(n = 1,min = 0,max = 1)<=pool.cross,fparam[actvars][sample.int(n = length(actvars),size =1)],fparam.pool[sample.int(n = length(fparam.pool),size =1,prob = pool.probs)])
                                           proposal<-stri_paste("I(",sigmas[sample.int(n = length(sigmas),size=1,replace = F,prob = sigmas.prob)],"(",proposal,"))",sep = "")
                                         }
                                       }else if(action.type==4){
                                         # modification type of a proposal of one of either currently active or all covariates
                                         ...
                                         # select a subset for the projection
                                         if(length(actvars)>0)
                                           actvars <- actvars[which(stats::rbinom(n = length(actvars),size = 1,prob = 0.5)==1)]
