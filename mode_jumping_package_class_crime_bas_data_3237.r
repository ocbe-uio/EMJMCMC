...
#prepare data
simx <- read.table(paste(workdir,"simcen-x1.txt",sep = "",collapse = ""),sep = ",")
simy <- read.table(paste(workdir,"simcen-y1.txt",sep = "",collapse = ""))
...
system.time({
  ...
  for(i in 1:Niter)
  {
    ...
    resm<-mySearch$modejumping_mcmc(list(varcur=initsol,statid=5, distrib_of_proposals = distrib_of_proposals,distrib_of_neighbourhoods=distrib_of_neighbourhoods, eps = 0.0001, trit = 32700, trest = 3230 , burnin = 50, max.time = 30, maxit = 100000, print.freq =100))
