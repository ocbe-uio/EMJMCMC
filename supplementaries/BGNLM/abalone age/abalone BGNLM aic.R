...
for(j in 1:100)
{
  ...
  write.csv(x =res1$feat.stat,row.names = F,file = paste0("posteriorshella_",j,".csv"))
  print(paste0("end simulation ",j))
  
  #print the run's metrics and clean the results
  write.csv(file =paste0("resultsruna_",j,".csv"),x= results[,j,])
  #rm(results)
...
for(j in 1:100)
{
  ...
  tmp = read.csv(paste0("resultsruna_",j,".csv"))
...
for(j in 1:100)
{
  tmpf = read.csv(paste0("posteriorshella_",j,".csv"))
...
write.csv(x =tmp,row.names = T,file = "abalonefeata.csv")
#print(paste0("end simulation ",j))
