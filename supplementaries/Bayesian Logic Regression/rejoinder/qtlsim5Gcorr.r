...
for(j in MM:80)
{
  tryCatch({
    ...
    m = clusterGeneration::rcorrmatrix(50,alphad=2.5) 
    ...
    X = bindata::rmvbin(1000, margprob = rep(0.5,50), bincorr = m)
    ...
    tryCatch({
      ...
      write.csv(x =res1,row.names = F,file = paste0("post2etaG_",j,".csv"))
    },error = function(err){
      print("error")
      write.csv(x =posteriors,row.names = F,file = paste0("posteriors2etaG_",j,".csv"))
    },finally = {
