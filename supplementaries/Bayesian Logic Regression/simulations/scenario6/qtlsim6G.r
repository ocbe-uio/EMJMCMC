...
estimate.logic.lm.tCCH = function(formula = Y1~-1., data, n=1000, m=50, r = 1, p.a = 1, p.b = 2, p.r = 1.5, p.s = 0, p.v=-1, p.k = 1)
{
  ...
  mlik = (-0.5*p*log(p.v) -0.5*(n-1)*log(1-(1-1/p.v)*R.2) + log(beta((p.a+p)/2,p.b/2)) + log(phi1(p.b/2,(n-1)/2,(p.a+p.b+p)/2,p.s/2/p.v,R.2/(p.v-(p.v-1)*R.2))) +log(Jprior) + p*log(r)+n) 
...
for(j in 1:MM)
{
  tryCatch({
    ...
    tryCatch({
      ...
      write.csv(x =res1,row.names = F,file = paste0("post3etaG_",j,".csv"))
    },error = function(err){
      print("error")
      write.csv(x =posteriors,row.names = F,file = paste0("posteriors3etaG_",j,".csv"))
    },finally = {
