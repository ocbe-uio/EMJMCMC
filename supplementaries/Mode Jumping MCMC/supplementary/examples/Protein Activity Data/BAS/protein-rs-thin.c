/*gcc -g -O2 -o protein-rs-thin.exe protein-rs-thin.c -llapack -lblas -lm */
...
main()
{
  ...
  int i,j,jseed,k,l,nrow = 96, p=88, ncolumn,  dimgam, prop, itno = p*pow(2,20) ,g=96, *indices, ok, *varin, *varout ;               
...
for (jseed=1; jseed<11; jseed++)
   {
 srand(jseed); 
 sprintf (filename, "protein-rs-thin.%d.dat",jseed);
 ...
 fmodelmat =   fopen(filename,"w");
  ...
  fpt = fopen("proteincen.txt","r");  // open file for reading only, remember last column of is y
    for (i=0; i<nrow; i++)
       {
	  for (j=0; j<p; j++)
	    {	      
