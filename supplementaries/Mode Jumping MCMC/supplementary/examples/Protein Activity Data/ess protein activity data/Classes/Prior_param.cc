...
void Prior_param::set_PR_param(int g_opt,
...
{
  if(g_opt==1){
    get_prior_parameters_g_prior(E_p_gam_from_read,
				 Sd_p_gam_from_read,
				 pX,
				 pY,
				 nX,
    ...
    get_prior_parameters_ind_prior(E_p_gam_from_read,
				   Sd_p_gam_from_read,
				   pX,
				   pY,
				   vect_RMSE,
...
void Prior_param::get_prior_parameters_g_prior(double E_p_gam_from_read,
					       double Sd_p_gam_from_read,
					       unsigned int pX,
					       unsigned int pY,
					       unsigned int nX,
...
void Prior_param::get_prior_parameters_ind_prior(double E_p_gam_from_read,
						 double Sd_p_gam_from_read,
						 unsigned int pX,
						 unsigned int pY,
						 gsl_vector *vect_RMSE,
						 ...
						 double P_DR_from_read)
{
  ...
  while(!stop){
    ...
    b=(pow((double)(Sd_p_gam),2.0)-
	  (double)(pX)*a*(1.0 - (double)(pX)*a)/((double)(pX)*(double)(pX - 1)*a));
