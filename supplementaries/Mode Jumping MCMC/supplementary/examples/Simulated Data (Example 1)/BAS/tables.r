...
multi.all <- bas.lm(y~.,data=multidata,n.models=NULL,prior="g-prior",modelprior= uniform(),update=500,
                    alpha=nrow(multidata),initprobs="eplogp")
true.incprob <- multi.all$probne0[-1];
truesummarg <- sum(exp(multi.all$logmarg))
fake500 <- sum(exp(sort(multi.all$logmarg,decreasing = TRUE)[1:700]))/truesummarg;
