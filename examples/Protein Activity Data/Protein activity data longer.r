...
system.time({
  ...
  for(i in 1:Niter)
  {
    ...
    resm<-mySearch$modejumping_mcmc(list(varcur=NULL,statid=-1, distrib_of_proposals =distrib_of_proposals,distrib_of_neighbourhoods=distrib_of_neighbourhoods, eps = 0.000000000001, trit = 2^50, trest = 2^23, burnin = 100, max.time = 24*60*6, maxit = 2^20, print.freq = 1000
    ))
    ...
    write(mliklist[,i], file = paste("mlikslong",i,".csv"),
          ncolumns = 1,
          append = FALSE, sep = " ")
    write(vect[,i], file = paste("pplong",i,".rs.csv"),
          ncolumns = 1,
          append = FALSE, sep = " ")
    write(vect.mc[,i], file = paste("pplong",i,".mc.csv"),
          ncolumns = 1,
          append = FALSE, sep = " ")
