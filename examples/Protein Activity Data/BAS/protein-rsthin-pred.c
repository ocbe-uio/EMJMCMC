// To complile:
/*gcc -g -O2 -o protein-rsthin-pred.exe protein-rsthin-pred.c -llapack -lblas -lm */

// To run
/* ./protein-rsthin-pred.exe case.caseno.txt*/
// where caseno is a generic notation for the case that we want to predict, for eg for caseno=2, do
/* ./protein-rsthin-pred.exe case.2.txt*/  

/* protein-rstshin-pred.c */
...
{
   FILE *fpt;         // define a pointer to a file for reading the data, centered after del-ing the ith case*/
  FILE *fmodelmat;   // define a pointer to a file for writing the array modelmat
  FILE *fcaseno;     // define a pointer to a file for reading the deleted case number*/
  int i,j,jseed,k,l,nrow = 95, p=88, ncolumn,  dimgam, prop, itno = p*pow(2,20) ,g=95, *indices, ok, *varin, *varout ; 
  int caseno;              
  ...
  char outputfile[50];
  char inputdatafile[50];
  char inputcasenofile[50];
 


    fcaseno=fopen(argv[1],"r");
    fscanf(fcaseno,"%d",& caseno);
           
  fclose(fcaseno); /* close the case no file*/
   
 
 sprintf(outputfile,"rsthin-pred.%d.dat",caseno);
 //printf ("Output filename is %s \n",outputfile);

 sprintf(inputdatafile,"proteindata.%d.cen.txt",caseno);
 //printf (Input protein data filename is %s \n",inputdatafile);

 srand(caseno); 
 
 
 fmodelmat =   fopen(outputfile,"w");
  /* First, call gettimeofday() to get start time */
 gettimeofday(&start_time, (struct timeval*)0);

  fpt = fopen(inputdatafile,"r");  // open file for reading only, remember last column of is y
    for (i=0; i<nrow; i++)
       {
	  for (j=0; j<p; j++)
	    {	      
