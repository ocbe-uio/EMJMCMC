/*gcc -g -O2 -o crime-rs-thin.exe crime-rs-thin.c -llapack -lblas -lm */
...
main()
{
  ...
  int i,j,jseed,k,l,nrow = 47, p=15, ncolumn,  dimgam, prop, itno = p*pow(2,15), g=47, *indices, ok, *varin, *varout ;               
...
for (jseed=1; jseed<2; jseed++)
   {
 srand(jseed); 
 sprintf (filename,"crime-rs-thin.%d.dat",jseed);
 ...
 fmodelmat =   fopen(filename,"w");
  ...
  /* First, call gettimeofday() to get start time */
   gettimeofday(&start_time, (struct timeval*)0);
    ...
    fpt = fopen("crimecen.txt","r");  // open file for reading only, remember last column is y
    for (i=0; i<nrow; i++)
       {
	  for (j=0; j<p; j++)
	    {	      
