/*gcc -g -O2 -o sim-rs.exe sim-rs.c -llapack -lblas -lm */
...
main()
{
  ...
  int i,j,jseed,k,l,nrow = 100, p=15, ncolumn, dimgam, prop, itno = 3276,g=100, *indices, *varin, *varout, ok ;               
 ...
 for (jseed=1; jseed<101; jseed++)
   {
 srand(jseed); 
 sprintf (filename,"sim-rs.%d.dat",jseed);
...
fpt = fopen("simcen-x.txt","r");  // open file for reading only
 ...
 for (i=1; i<itno; i++)       // starting iterations for MH Algorithm
    ...
    varout = ivector(p-dimgam) ;//vector of excluded variables in current model (don't consider intercept)
     ...
     indices = ivector(ncolumn) ;//indices  indicates the column position of the nonzero gamma_j's, doesn't include intercept
             ...
             } else
	 {
	   ...
	   logmarggammanew = .5*((double)(log(1.0 + (double)g)) * (double) (nrow - ncolumn - 1)
