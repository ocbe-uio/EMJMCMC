...
data.example = data.example[,c(59,1:58)]
...
for(ii in 1:100)
{
 ...
 # capture.output({withRestarts(tryCatch(capture.output({
    ...
    results[3,ii,1]= sqrt(mean((out - test$shares)^2))
    results[3,ii,2]=mean(abs(out - test$shares))
    results[3,ii,3] = cor(out,test$shares)
    ...
    #compute and store the performance metrics
    #compute and store the performance metrics
    results[4,ii,1]= sqrt(mean((out - test$shares)^2))
    results[4,ii,2]=mean(abs(out - test$shares))
    results[4,ii,3] = cor(out,test$shares)
    ...
    #compute and store the performance metrics
    results[5,ii,1]= sqrt(mean((out - test$shares)^2))
    results[5,ii,2]=mean(abs(out - test$shares))
    results[5,ii,3] = cor(out,test$shares)
    ...
    #compute and store the performance metrics
    results[6,ii,1]= sqrt(mean((out - test$shares)^2))
    results[6,ii,2]=mean(abs(out - test$shares))
    results[6,ii,3] = cor(out,test$shares)
    ...
    #compute and store the performance metrics
    results[10,ii,1]= sqrt(mean((out - test$shares)^2))
    results[10,ii,2]=mean(abs(out - test$shares))
    results[10,ii,3] = cor(out,test$shares)
    ...
    #compute and store the performance metrics
    results[7,ii,1]= sqrt(mean((out - test$shares)^2))
    results[7,ii,2] = mean(abs(out - test$shares))
    results[7,ii,3] = cor(out,test$shares)
    
    

    #h2o deeplearning
    t=system.time({
      neo.dl = h2o.deeplearning(x = features, y = "shares",hidden=c(200,200,200,200,200,200),
    ...
    results[8,ii,5]=t[3]
    out=as.data.frame(as.matrix(out))$predict
    results[8,ii,1]= sqrt(mean((out - test$shares)^2))
    results[8,ii,2] = mean(abs(out - test$shares))
    results[8,ii,3] = cor(out,test$shares)
    
    
    #h2o glm
    t=system.time({
      neo.glm = h2o.glm(x = features, y = "shares",
                        family = "gaussian",
                        training_frame = train1,
                        validation_frame = valid1,
                        #lambda = 0,
    ...
    results[9,ii,5]=t[3]
    out=as.data.frame(as.matrix(out))$predict
    results[9,ii,1]= sqrt(mean((out - test$shares)^2))
    results[9,ii,2] = mean(abs(out - test$shares))
    results[9,ii,3] = cor(out,test$shares)
