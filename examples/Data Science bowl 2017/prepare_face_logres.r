...
scansPath <- "scans/" 
picdir <- "results/scans/face/"
resultLabelsPath<-"results/labels/face/"
resultLabelsFile = paste0(resultLabelsPath, "data_face.csv")
labelsPath <- "labels.csv"
patientLabelsPath <- "results/labels/face/"
...
proceed.head<-function(pat.id)
{
  ...
  hval = hist(dicom2nifti,breaks = 15)$density[1:7]
  slope = scl_slope(dicom2nifti)
  intercept = scl_inter(dicom2nifti)
  
  vect<-array(data = list(),dim = dim(dicom2nifti)[2])
  for(i in 1:dim(dicom2nifti)[2])
  {
    ...
    vect[[i]]$size.x = dim(dicom2nifti)[1]
  ...
  if(length(ocsv)!=0)
    write.csv(file = paste0(patientLabelsPath, "data_face_",pat.id,"_",imax,".csv"),x = ocsv)
  ...
  png(paste0(picdir,"nodules_f_",pat.id,"_",stat,".png"), width=512, height=512)
  image(res[[imax]]$nodules, useRaster = F,col = grey(0:64 / 64)) # should not suffer from anti-aliasing
  if(doPNG)
    dev.off()
  if(doPNG <- !dev.interactive())
    png(paste0(picdir,"lungs_f_",pat.id,"_",stat,".png"), width=512, height=512)
  image(res[[imax]]$lungs, useRaster = F,col = grey(0:64 / 64)) # should not suffer from anti-aliasing
  if(doPNG)
    dev.off()
  if(doPNG <- !dev.interactive())
    png(paste0(picdir,"img_f_",pat.id,"_",stat,".png"), width=512, height=512)
