...
gant<-function(x,q)as.integer(x<quantile(x,probs = q))
...
for(i in 1:20)
{
  for(j in 1:3)
  {
    df<-cbind(df,gant(voice[,i],0.25*j))
  }

}
df<-as.data.frame(df)
names(df)[1]<-"label"
index <- createDataPartition(df$label, p = 0.25, list = FALSE)
...
for(ii in 1:M)
{
  ...
  t<-system.time({
    ...
    res<-mySearch$forecast.matrix.na(link.g = g,covariates = (test[,-1]),betas = betas,mliks.in = mliks)$forecast
...
# }
 ...
 # test<-as.data.frame(test[,-c(21)])
  ...
  t<-system.time({
  fit2 <- glmnet(as.matrix(train)[,-1], train$label, family="binomial")
  })
  results[5,ii,4]<-t[3]
  ...
  t<-system.time({
    fit2 <- glmnet(as.matrix(train)[,-1], train$label, family="binomial",alpha=0)
  })
  results[6,ii,4]<-t[3]
