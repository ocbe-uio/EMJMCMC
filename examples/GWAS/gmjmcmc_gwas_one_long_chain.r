...
for(j in 1:1)
{
  tryCatch({
    ...
    for(i in 1:M)
    {
      ...
      pheno<-read.csv(paste0("data_S2_nocausal_5402/pimass/data.recode.pheno_",i,".txt"),header = F)
      ...
      #cov.names<-names(sum$coefficients[-1,4])
      #params[[i]]$
      params[[i]]$secondary <- secondary 
      params[[i]]$data=data.example
      params[[i]]$estimator.args =  list(data = data.example)
    ...
    results<-parall.gmj(X = params, M = 31)
    #print(results)
