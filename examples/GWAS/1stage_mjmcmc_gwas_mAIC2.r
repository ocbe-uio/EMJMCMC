...
for(j in 51:99)
{
  #tryCatch({
    ...
    write.csv(x =c(herl,her,heru),row.names = F,file = paste0("MJRES/herest11ASMJSIM_",j,".csv"))

    KMK<-which(max.popul==max(max.popul,na.rm = T))[[1]]

    write.csv(x =c(results[[KMK]]$cterm,results[[KMK]]$best),row.names = F,file = paste0("MJRES/bestmod11ASMJSIM_",j,".csv"))
    ...
    #tryCatch({
      ...
      write.csv(x =res1,row.names = F,file = paste0("MJRES/post11ASMJSIM_",j,".csv"))
    #},error = function(err){
    #  print("error")
