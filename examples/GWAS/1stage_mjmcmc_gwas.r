...
pheno<-read.csv(paste0("data_S1_nocausal_5402/pimass/data.recode.pheno_",1),header = F)
geno<-t(read.csv("data_S1_nocausal_5402/pimass/data.recode.mean.geno.txt",header = F,stringsAsFactors = F))
names<-geno[1,]
...
simplifyposteriors<-function(posteriors,th=0.0001,thf=0.2, dataNeigbourhood = dataNeigbourhoodS1)
{
...
for(j in 1:100)
{
  tryCatch({
    ...
    pheno<-read.csv(paste0("data_S1_nocausal_5402/pimass/data.recode.pheno_",j),header = F)
    ...
    detect.true.unique<-unique(dataNeigbourhoodS1$causSNPid[which(dataNeigbourhoodS1$SNPid %in% detected)])
    detect.true<-which(detected %in% dataNeigbourhoodS1$SNPid)
    ...
    vect<-list(formula = formula1, locstop.nd = T, keep.origin = F,p.add = 0.1,max.time = 75, p.add.default = 0.1, pool.cor.prob = T,secondary <-names[-which(names %in% cov.names)], outgraphs=F,data = data.example,estimator = estimate.lm.MBIC2,presearch=T, locstop =F,estimator.args =  list(data = data.example),recalc_margin = 999,gen.prob = c(1,0,0,0,0), save.beta = F,interact = F,relations=c("cos"),relations.prob =c(0.1),interact.param=list(allow_offsprings=3,mutation_rate = 1000, last.mutation = 15000, max.tree.size = 4, Nvars.max =(compmax-1),p.allow.replace=0.7,p.allow.tree=0.25,p.nor=0,p.and = 0.9),n.models = 25000,unique = T,max.cpu = 4,max.cpu.glob = 4,create.table = F,create.hash = T,pseudo.paral = T,burn.in = 50,print.freq = 1000,advanced.param = list(
    ...
    write.csv(x =c(herl,her,heru),row.names = F,file = paste0("herestcmjmcbic1_",j,".csv"))
    
    KMK<-which(max.popul==max(max.popul,na.rm = T))
    
    write.csv(x =c(results[[KMK]]$cterm,results[[KMK]]$best),row.names = F,file = paste0("bestmodcmjmcbic1_",j,".csv"))
    ...
    tryCatch({
      ...
      write.csv(x =res1,row.names = F,file = paste0("MJRES/post11SMJSIM_",j,".csv"))
    },error = function(err){
      print("error")
      write.csv(x =posteriors,row.names = F,file = paste0("MJRES/post11SEGMJSIM_",j,".csv"))
    },finally = {
