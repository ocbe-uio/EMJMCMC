...
for(j in 1:100)
{
  tryCatch({
    ...
    write.csv(x =c(herl,her,heru),row.names = F,file = paste0("herestcmjmcbic4_",j,".csv"))
    
    KMK<-which(max.popul==max(max.popul,na.rm = T))
    
    write.csv(x =c(results[[KMK]]$cterm,results[[KMK]]$best),row.names = F,file = paste0("bestmodcmjmcbic4_",j,".csv"))
    ...
    tryCatch({
      ...
      write.csv(x =res1,row.names = F,file = paste0("MJRES/post14SMJSIM_",j,".csv"))
    },error = function(err){
      print("error")
      write.csv(x =posteriors,row.names = F,file = paste0("MJRES/post14SEGMJSIM_",j,".csv"))
    },finally = {
