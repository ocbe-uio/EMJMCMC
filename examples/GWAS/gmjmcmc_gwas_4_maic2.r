...
#dataNeigbourhoodS3 <- plyr::ldply(causSNPsS4, findNeigbour)
...
pheno<-read.csv(paste0("data_S4_nocausal_5402/pimass/data.recode.pheno_",1,".txt"),header = F)
...
estimate.lm.MAIC2 <- function(formula, data, n = 5402, m = 24572, c = 4,u=170)
{
...
for(j in 1:100)
{
  tryCatch({
    ...
    pheno<-read.csv(paste0("data_S4_nocausal_5402/pimass/data.recode.pheno_",j,".txt"),header = F)
    data.example$Y<-as.numeric(pheno$V1)
    rm(pheno)
    cors<-cor(data.example$Y,data.example[,1:24572])
    gc()
    ...
    vect<-list(formula = formula1, locstop.nd = T, keep.origin = F,p.add = 0.1,max.time = 180, p.add.default = 0.1, pool.cor.prob = T,secondary <-names1[-which(names1 %in% cov.names)], outgraphs=F,data = data.example,estimator = estimate.lm.MAIC2,presearch=F, locstop =T,estimator.args =  list(data = data.example),recalc_margin = 999,gen.prob = c(1,0,0,0,0), save.beta = F,interact = T,relations=c("cos"),relations.prob =c(0.1),interact.param=list(allow_offsprings=3,mutation_rate = 1000, last.mutation = 15000, max.tree.size = 4, Nvars.max =(compmax-1),p.allow.replace=0.7,p.allow.tree=0.25,p.nor=0,p.and = 0.9),n.models = 25000,unique = T,max.cpu = 4,max.cpu.glob = 4,create.table = F,create.hash = T,pseudo.paral = T,burn.in = 50,print.freq = 1000,advanced.param = list(
      max.N.glob=as.integer(90),
    ...
    tryCatch({
      ...
      write.csv(x =res1,row.names = F,file = paste0("post4AGMJSIM_",j,".csv"))
    },error = function(err){
      print("error")
      write.csv(x =posteriors,row.names = F,file = paste0("post4EGMJSIM_",j,".csv"))
    },finally = {
