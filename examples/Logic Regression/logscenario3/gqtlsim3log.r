...
estimate.logic.bern.tCCH.alt <- function(formula = NULL,y.id = 51, data, n=1000, m=50, r = 1, p.a = 1, p.b = 2, p.r = 1.5, p.s = 0, p.v=-1, p.k = 1)
{
  if(is.null(formula))
    return(list(mlik =  -10000 + rnorm(1,0,1),waic =10000 , dic =  10000,summary.fixed =list(mean = 1)))
  X <- scale(model.matrix(object = formula,data = data),center = T,scale = F)
  ...
  mlik = (logLik(out) - 0.5*log(J.a.hat) + 0.5*log(p.v) - 0.5*(p+1)*log(Q/2) + log(gammainc(x = Q/2/p.v, a = (p+1)/2)[1])+Jprior +n)
          if(length(mlik)==0||mlik==-Inf||mlik==Inf)
            mlik = -10000+ rnorm(1,0,1)
  return(list(mlik = mlik,waic = AIC(out) , dic =  BIC(out),summary.fixed =list(mean = coefficients(out))))
}
...
for(j in 69:91)
{
  ...
  results<-mclapply(X = params,FUN = runpar,mc.preschedule = FALSE, mc.cores = 32)
  gc()
  wait()
  ...
  tryCatch({
    ...
    write.csv(x =res1,row.names = F,file = paste0("postLog3etaG_",j,".csv"))},error = function(err){
      print("error")
      write.csv(x =posteriors,row.names = F,file = paste0("posteriorsLog3etaG_",j,".csv"))},finally = {
