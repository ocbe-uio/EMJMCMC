...
for(j in 77:MM)
{
 ...
 X1<- as.data.frame(array(data = rbinom(n = 50*1000,size = 1,prob = 0.3),dim = c(1000,50)))
  ...
  tryCatch({
    ...
    write.csv(x =res1,row.names = F,file = paste0("postLog2etaOld_",j,".csv"))},error = function(err){
      print("error")
      write.csv(x =posteriors,row.names = F,file = paste0("posteriorsLog2etaOld_",j,".csv"))},finally = {
