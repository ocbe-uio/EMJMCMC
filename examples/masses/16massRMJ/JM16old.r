...
source("https://raw.githubusercontent.com/aliaksah/EMJMCMC2016/master/R/the_mode_jumping_package5.r")
...
estimate.gamma.cpen <- function(formula = NULL, data,r = 1.0/223.0,logn=log(223.0),distribution = "normal",link = "identit",relat=c("sigmoid","exp","logi"))
{
  fparam<-NULL
  mlik = -10000
  waic = 10000
...
gnlr = function(y = data$PlanetaryMassJpt,  distribution = "normal", mu,pmu)gnlm::gnlr(y = y, distribution =distribution,mu = mu,pshape = 1,pmu = pmu)

identit = function(x)x
...
for(j in 1:100)
{
  tryCatch({
    ...
    formula1 = as.formula(paste(colnames(X)[2],"~ 1 +",paste0(colnames(X)[-2],collapse = "+")))
    ...
    vect<-list(formula = formula1,data = data.example,estimator = estimate.gamma.cpen,estimator.args =  list(distribution = "normal",link = "identit",data = data.example),recalc_margin = 249,deep.method = 3, save.beta = F,interact = T,outgraphs=F,relations=c("exp","logi","sigmoid"),relations.prob =c(0.1,0.1,0.1),interact.param=list(allow_offsprings=3,mutation_rate = 250,last.mutation=10000, max.tree.size = 5, Nvars.max =15,p.allow.replace=0.9,p.allow.tree=0.01,p.nor=0.9,p.and = 0.9),n.models = 10000,unique =T,max.cpu = 4,max.cpu.glob = 4,create.table = F,create.hash = T,pseudo.paral = T,burn.in = 100,print.freq = 1000,advanced.param = list(
    ...
    tryCatch({
      ...
      write.csv(x =res1,row.names = F,file = paste0("postJM16old_",j,".csv"))
    },error = function(err){
      print("error")
      write.csv(x =posteriors,row.names = F,file = paste0("posteriorsJM16old_",j,".csv"))
    },finally = {
